 ...
 *
 * For each fixture file (investigation docs + synthetic network-survey-style
 * pages), asserts:
...
// ---------------------------------------------------------------------------
// Synthetic network-survey-style fixtures
// ---------------------------------------------------------------------------

const NETWORK_FIXTURES: string[] = [
  "network-overview.md",
  "network-account-detail.md",
  "network-traffic-flows.md",
];
...
// ---------------------------------------------------------------------------
// Suite 5: network-survey-style synthetic fixtures
// ---------------------------------------------------------------------------

describe("network-survey synthetic fixtures — markdownToStorage", () => {
  for (const fixtureName of NETWORK_FIXTURES) {
    it(`converts ${fixtureName} without error`, () => {
      const md = loadFixture(fixtureName);
      assertValidStorageOutput(md, fixtureName);
    });
...
describe("network-survey synthetic fixtures — structural assertions", () => {
  it("overview fixture produces tables and headings", () => {
    const md = loadFixture("network-overview.md");
    const storage = markdownToStorage(md);
  ...
  it("account-detail fixture produces code blocks as Confluence macros", () => {
    const md = loadFixture("network-account-detail.md");
    const storage = markdownToStorage(md);
  ...
  it("traffic-flows fixture produces warning panel macro", () => {
    const md = loadFixture("network-traffic-flows.md");
    const storage = markdownToStorage(md);
...
describe("network-survey synthetic fixtures — token round-trip", () => {
  for (const fixtureName of NETWORK_FIXTURES) {
