...
describe("fenced code blocks", () => {
  ...
  it("preserves angle-bracketed placeholders inside a no-language fence and closes the macro before following content", () => {
    const md =
      "```\n" +
      "alias/example/secrets/<service>         (per-service)\n" +
      "alias/example/secrets/tenants/<tenant-id>   (per-tenant)\n" +
      "```\n\n" +
...
describe("links", () => {
  ...
  it("emits a plain <a href> anchor for internal Confluence URLs (B2)", () => {
    ...
    const url = `${BASE_URL}/wiki/spaces/DEV/pages/875954196/EX-1234+Overview`;
  ...
  it("rewrites confluence:// scheme to ac:link with space-key and content-title", () => {
    const out = convert("[Overview](confluence://DEV/EX-1234 Overview)", {
      confluenceBaseUrl: BASE_URL,
    });
    expect(out).toContain("<ac:link>");
    expect(out).toContain('<ri:page ri:space-key="DEV" ri:content-title="EX-1234 Overview"');
  ...
  it("does NOT rewrite external link that looks like confluence URL from different host", () => {
    const spoofUrl = "https://example.atlassian.net.attacker.com/wiki/spaces/DEV/pages/12345/foo";
  ...
  it("does NOT rewrite links when no confluenceBaseUrl is configured", () => {
    const url = `${BASE_URL}/wiki/spaces/DEV/pages/12345/Foo`;
...
describe("URL-spoofing regression", () => {
  it("does not rewrite subdomain-extended URL as internal", () => {
    const spoof = "https://example.atlassian.net.attacker.com/wiki/spaces/X/pages/99/Foo";
    const out = convert(`[Foo](${spoof})`, { confluenceBaseUrl: BASE_URL });
    expect(out).not.toContain("<ac:link>");
  });

  it("does not rewrite URL with different port as internal", () => {
    const diffPort = "https://example.atlassian.net:8080/wiki/spaces/X/pages/99/Foo";
  ...
  it("treats URL with explicit port 443 as internal (plain <a> after B2)", () => {
    const explicit443 = "https://example.atlassian.net:443/wiki/spaces/X/pages/12345/Foo";
