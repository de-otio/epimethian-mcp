...
const DRAWIO_MACRO = [
  ...
  `  <ac:parameter ac:name="baseUrl">https://example.atlassian.net/wiki</ac:parameter>`,
...
describe("security: URL spoofing end-to-end", () => {
  ...
  it("spoofed host is treated as external link, not ac:link", () => {
    const md = `[Link](https://example.atlassian.net.attacker.com/wiki/spaces/X/pages/123)`;
  ...
  it("real Confluence URL is emitted as a plain <a href> anchor (B2)", () => {
    const url = "https://example.atlassian.net/wiki/spaces/DEV/pages/12345";
    const md = `[Link](${url})`;
    const storage = markdownToStorage(md, { confluenceBaseUrl: BASE_URL });
