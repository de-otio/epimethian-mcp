...
- `example.atlassian.net.attacker.com` — different host, must not match.
- `attacker.com:443/...?host=example.atlassian.net` — different host, must not match.
- `example.atlassian.net:8080` — same host, different port: decide explicitly (recommend match if scheme+host+port-or-default match).
- Percent-encoded host (`%65xample.atlassian.net`) — must canonicalise before comparison.
- `userinfo@host` (`anything@example.atlassian.net`) — must compare host, not authority.
