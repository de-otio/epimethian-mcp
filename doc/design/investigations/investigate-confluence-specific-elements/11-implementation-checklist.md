...
- [ ] `url-parser.ts`:
  ...
  - [ ] Tests: spoofing variants from [Security #5](06-security.md#5-url-rewrite-spoofing) — `example.atlassian.net.attacker.com`, port-suffix tricks, percent-encoded host (`%65xample.atlassian.net`), `userinfo@host` (`anything@example.atlassian.net`), `?host=...` query trickery, IPv6 hosts.
...
- [ ] **Synthetic real-world fixtures:** capture sample outputs of `generate_confluence_page`-style content (overview, account, traffic-flows) into fixtures; assert round-trip.
